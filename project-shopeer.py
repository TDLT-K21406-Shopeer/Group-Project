...
def chi_tieu(stcl,thong_ke, tongtien):
    ...
    print('Nhập chi phí phải bỏ ra (đơn vị là VNĐ): ')
    cp=nhap_du_lieu_so()
    if cp>stcl:
        print('Chi phí lớn hơn hạn mức mất rồi, bạn cần lựa chọn:')
        print(' \n1.Từ bỏ việc chi tiêu này')
        print('2.Mở rộng hạn mức để đủ cho chi phí này')
...
#--------------------------------------
print('Chương trình quản lý chi tiêu của nhóm Shopeer')
s=str(input('Nhập tên người dùng: '))
print('Chào mừng {} đã đến với chương trình của Shopeer'.format(s))
...
vuot_hm=0
for i in range(1,31):
    #Lấy trung bình mỗi tháng 30 ngày
    print('Ngày thứ',i)
    kt=False
    while kt==False:
...
if dem==0:
    if stcl > 0:
        print('THÔNG BÁO: Bạn đã chi tiêu ít hơn hạn mức:',stcl) 
        print('Chúc mừng {} đã có một tháng chi tiêu tiết kiệm'.format(s))
    elif stcl == 0:
